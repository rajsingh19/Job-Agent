...
from app.services.connectors.capabilities import get_ashby_capabilities


class AshbyConnector(PlatformConnector):
    """
    Connector for Ashby ATS platform (jobs.ashbyhq.com / ashbyhq.com).
    """

    @property
    def platform(self) -> PlatformType:
        return PlatformType.ASHBY

    @property
    def name(self) -> str:
        return "Ashby ATS Connector"

    @property
    def description(self) -> str:
        return "Native ATS connector for Ashby job board listings and application routing."
    ...
    async def get_application_route(
        ...
        signals = detection_result.signals if detection_result else ["connector=AshbyConnector"]
